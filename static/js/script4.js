...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.getElementById("calculator-form").addEventListener("submit", async function (event) {
        ...
        let taskTime = parseFloat(document.getElementById("task-time").value);
        let tasksPerMonth = parseFloat(document.getElementById("tasks-per-month").value);
        let hourlyRate = parseFloat(document.getElementById("hourly-rate").value);
    
        let response = await fetch("/calculate_savings", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ task_time: taskTime, tasks_per_month: tasksPerMonth, hourly_rate: hourlyRate })
        });
    
        let data = await response.json();
    
        document.getElementById("current-time").textContent = data.current_time;
        document.getElementById("automated-time").textContent = data.automated_time;
        document.getElementById("time-saved").textContent = data.time_saved;
        document.getElementById("cost-saved").textContent = data.cost_saved;
    ...
    // Zorg dat alle sliders correct werken
    function updateValue(id, value, prefix = "", suffix = "") {
        document.getElementById(id).textContent = prefix + value + suffix;
    }

    // Sliders ophalen
    let taskTime = document.getElementById("task-time");
    let tasksPerMonth = document.getElementById("tasks-per-month");
    let hourlyRate = document.getElementById("hourly-rate");

    // Event Listeners toevoegen
    taskTime.addEventListener("input", () => updateValue("task-time-display", taskTime.value, "", " min"));
    tasksPerMonth.addEventListener("input", () => updateValue("tasks-per-month-display", tasksPerMonth.value));
    hourlyRate.addEventListener("input", () => updateValue("hourly-rate-display", hourlyRate.value, "€"));

    console.log("✅ Sliders zijn correct geladen en werken!");

    
    function exportPDF() {
        let element = document.getElementById("results");
        html2pdf().from(element).save("Kostenbesparing-Rapport.pdf");
    }
