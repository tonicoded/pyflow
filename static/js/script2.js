...
// 🚀 **Live Dashboard Functionaliteit**
function setupLiveDashboard() {
    function updateDashboard() {
        fetch('/dashboard-data')
            .then(response => response.json())
            .then(data => {
                animateNumberChange("cost-saving", data.cost_saving);
                animateNumberChange("time-saved", data.time_saved);
                animateNumberChange("tasks-automated", data.tasks_automated);
                animateNumberChange("server-load", data.server_load);
                animateNumberChange("efficiency-improvement", data.efficiency_improvement);
                animateNumberChange("error-reduction", data.error_reduction);
            });
    }
    ...
    // ✅ **Smooth Getal Animatie**
    function animateNumberChange(id, newValue) {
        let element = document.getElementById(id);
        if (!element) return;

        let currentValue = parseFloat(element.innerText.replace(/[^\d.-]/g, '')) || 0;
        let step = (newValue - currentValue) / 40;
...
function fetchAutomationOutput() {
    fetch('/live-automation')
        .then(response => response.text())
        .then(data => {
            const outputElement = document.getElementById('automation-output');
            outputElement.innerHTML = "<p>" + data + "</p>";
        });
}

setInterval(fetchAutomationOutput, 3000); // Elke 3 sec vernieuwen
